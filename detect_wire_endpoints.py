...
def detect_wire_endpoints(
    ...
    # 2. 加粗 → 骨架化 → 断点缝合
    skel = _bridge_gaps(_thicken_wires(mask))

    # 3. 端点配对与合并
    pairs = merge_wires_with_threshold(_pair_endpoints(skel), max_dist=20)
    ...
    for (x1, y1), (x2, y2) in pairs:
        ...
        mid = (x1 + x2) // 2, (y1 + y2) // 2
        cv2.putText(vis, f"{dist:.1f}", mid,
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

    # 只弹这一张图
    #cv2.imshow("Wire Endpoints on Original Image", vis)
    #cv2.waitKey(0)
